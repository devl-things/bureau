...
export function HousekeepingPage(props: Readonly<Props>): React.ReactElement {
    ...
    async function load(): Promise<void> {
        ...
        try {
            const result = await choresApi.getHousekeepingLogs({
                page,
                pageSize,
                search: search.trim()
            });
    ...
    async function submitCreateOrEdit(): Promise<void> {
        ...
        try {
            ...
            if (modal === "create") {
                await choresApi.createHousekeepingLog(payload);
            } else if (modal === "edit" && active?.id) {
                await choresApi.updateHousekeepingLog(active.id, payload);
            }
    ...
    async function confirmDelete(): Promise<void> {
        ...
        try {
            await choresApi.deleteHousekeepingLog(active.id);
            closeModal();
            await load();
        } catch (e: unknown) {
            setError(toErrorMessage(e));
