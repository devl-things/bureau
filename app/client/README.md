# Frontend Workspace Structure (`app/client/`)
...
The `app/client/` workspace contains **all frontend source code** that is not implemented directly as Razor or server-rendered UI inside ASP.NET projects.
...
* mounting frontend bundles produced by `app/client/apps/*`

Frontend code in `app/client/`:
...
2. Frontend code stays in `app/client/`
...
The `app/client/` workspace provides a **clean, scalable frontend foundation** that supports:
