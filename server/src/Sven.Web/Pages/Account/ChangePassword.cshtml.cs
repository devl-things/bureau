...
namespace Sven.Pages.Account
{
    public class ChangePasswordModel : AuthPageModel
    {
        ...
        public ChangePasswordModel(ILogger<ChangePasswordModel> logger, ICurrentUserProvider currentUserProvider,
            AccountTranslations translations, IUserService userService) : base(currentUserProvider)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> OnPostAsync(CancellationToken cancellationToken = default)
        {
            ...
            // #54
            Result result = await _userService.UpdatePasswordAsync(CurrentUser.SubjectId, Input.Password!, cancellationToken);
            if (result.IsError)
            {
